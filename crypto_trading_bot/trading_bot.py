...
# Версия 50.0 - 12+ стратегий с адаптивным выбором под режим рынка
...
class BaseStrategy:
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        """Анализирует данные и возвращает сигнал или None"""
        raise NotImplementedError
    
    def get_signal_strength(self, data: Dict) -> float:
        """Возвращает силу сигнала от 0 до 1"""
        raise NotImplementedError
...
class ADXStrategy(BaseStrategy):
    """Стратегия на основе ADX и направленных индикаторов"""
    def __init__(self):
...
class RSIDivergenceStrategy(BaseStrategy):
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        ...
        # Ищем локальные минимумы и максимумы цены и RSI
        ...
        # БЫЧЬЯ ДИВЕРГЕНЦИЯ: цена делает更低 минимум, RSI -更高 минимум
        ...
        # МЕДВЕЖЬЯ ДИВЕРГЕНЦИЯ: цена делает更高 максимум, RSI -更低 максимум
    ...
    def get_signal_strength(self, data: Dict) -> float:
        ...
        # Сила сигнала: экстремальные значения RSI дают более сильные сигналы
        if rsi < 30:
            return (30 - rsi) / 30
        elif rsi > 70:
            return (rsi - 70) / 30
...
class BollingerBandsStrategy(BaseStrategy):
    """Стратегия отбоя от границ Боллинджера (боковик)"""
    def __init__(self):
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        ...
        # Полосы сужаются? (низкая волатильность - возможный breakout)
        ...
        # Отбой от нижней границы (покупка)
        if price <= curr_lower * 1.002 and prev_price > curr_lower:
            confidence = 0.65
            if bb_squeeze:
                confidence += 0.1
            if curr_middle > curr_lower * 1.05:  # Тренд вверх
                confidence += 0.05
        ...
        # Отбой от верхней границы (продажа)
        if price >= curr_upper * 0.998 and prev_price < curr_upper:
            confidence = 0.65
            if bb_squeeze:
                confidence += 0.1
            if curr_middle < curr_upper * 0.95:  # Тренд вниз
                confidence += 0.05
...
class VolumeBreakoutStrategy(BaseStrategy):
    """Стратегия на пробой с высоким объемом"""
    def __init__(self):
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        ...
        # Ищем уровни сопротивления/поддержки
        highs = np.array(data['highs'])
        lows = np.array(data['lows'])
        ...
        # Пробой сопротивления на высоком объеме (покупка)
        ...
        # Пробой поддержки на высоком объеме (продажа)
...
class CandlePatternStrategy(BaseStrategy):
    """Стратегия на свечных паттернах"""
    def __init__(self):
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        ...
        # Бычьи паттерны
        # 1. Бычье поглощение
        engulfing_bull = talib.CDLENGULFING(opens, highs, lows, closes)
        if engulfing_bull[-1] == 100:
            return {
                'action': 'BUY',
                'confidence': 0.75,
        ...
        engulfing_bear = talib.CDLENGULFING(opens, highs, lows, closes)
        if engulfing_bear[-1] == -100:
            return {
                'action': 'SELL',
                'confidence': 0.75,
    ...
    def get_signal_strength(self, data: Dict) -> float:
        # Для свечных паттернов сила сигнала определяется по месту паттерна
        closes = np.array(data['closes'])
        if len(closes) < 20:
            return 0.0
        
        # Чем ближе к экстремуму, тем сильнее сигнал
        price = closes[-1]
        recent_high = max(closes[-20:])
        recent_low = min(closes[-20:])
        
        if price > recent_high * 0.95:
            return 0.7  # Возможный разворот от максимума
        elif price < recent_low * 1.05:
            return 0.7  # Возможный разворот от минимума
        
        return 0.5

# ==================== СТРАТЕГИЯ 7: SUPPORT RESISTANCE ====================
class SupportResistanceStrategy(BaseStrategy):
    """Стратегия отбоя от уровней поддержки/сопротивления"""
    def __init__(self):
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        ...
        for level in levels:
            ...
            # Отбой от сопротивления (продажа)
...
class FibonacciStrategy(BaseStrategy):
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        ...
        # Находим значимые максимум и минимум за последние 50 свечей
        ...
        for level in self.fib_levels:
            if is_uptrend:
                ...
                # Откат к уровню Фибоначчи (покупка)
                ...
                # Откат к уровню Фибоначчи (продажа)
                if abs(price - fib_price) / price < 0.005:
                    confidence = 0.65
                    if level == 0.618 or level == 0.786:
                        confidence += 0.1
    ...
    def get_signal_strength(self, data: Dict) -> float:
        return 0.5  # Базовая сила для Фибоначчи

# ==================== СТРАТЕГИЯ 9: MACD CROSSOVER ====================
class MACDStrategy(BaseStrategy):
    """Стратегия на пересечении MACD"""
    def __init__(self):
...
class IchimokuStrategy(BaseStrategy):
    ...
    def analyze(self, data: Dict) -> Optional[Dict]:
        ...
        # Цена под облаком (медвежий сигнал)
        if price < senkou_a and price < senkou_b and tenkan < kijun:
            confidence = 0.7
            if senkou_a < senkou_b:
                confidence += 0.1
...
class MeanReversionStrategy(BaseStrategy):
    """Стратегия возврата к среднему (Z-score)"""
    def __init__(self):
...
class StrategyManager:
    """Управляет всеми стратегиями и выбирает лучший сигнал"""
    def __init__(self):
    ...
    def select_strategies_for_regime(self, regime: MarketRegime) -> List[str]:
        """Выбирает стратегии под текущий режим рынка"""
        strategy_map = {
    ...
    def get_best_signal(self, data: Dict, symbol: str) -> Optional[Dict]:
        """Получает лучший сигнал от всех стратегий"""
...
# ==================== ОСТАЛЬНЫЕ КЛАССЫ (BybitClient, AccountManager, MLSystem, AISystem, TradingBot) ====================
# [ЗДЕСЬ ДОЛЖНЫ БЫТЬ ОСТАЛЬНЫЕ КЛАССЫ, НО ДЛЯ ЭКОНОМИИ МЕСТА ПРОДОЛЖЕНИЕ В СЛЕДУЮЩЕМ СООБЩЕНИИ]
...
# ==================== ТОРГОВАЯ СТРАТЕГИЯ (ОСНОВНОЙ КЛАСС) ====================
class Strategy:
    ...
    def get_market_data(self, symbol):
        """Собирает все необходимые рыночные данные"""
    ...
    def calculate_technical_indicators(self, market_data):
        """Рассчитывает технические индикаторы для сигнала"""
    ...
    def find_entry_signal(self, symbol, acc, coin_analyzer):
        """Находит сигнал на вход используя менеджер стратегий"""
        ...
        # Получаем лучший сигнал от стратегий
            ...
            for trade in a.history:
                ...
                if entry_price:
                    ...
                    if closed:
                        ...
                        # Отправляем email уведомление о закрытии при синхронизации
...
"""
    ...
    async def open_trade(self, name, sig):
        ...
        if not confirm:
            logger.info(f"AI отклонил {sym}: {ai_score:.2f} - {ai_reason[:300]}")
            return
        
        # Применяем AI рекомендации по SL/TP если они есть
        if ai_stop_pct is not None and ai_tp_pct is not None:
            ...
            # Пересчитываем размер позиции с новым стопом
            size, risk_percent, risk_amount, lev = s.position_size(
                sym, sig["price"], sig["stop_pct"], sig["action"], a, False
            )
            sig["size"] = size
